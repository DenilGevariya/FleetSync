...
export default function FuelLogs() {
  ...
  const validate = () => {
    ...
    if (!form.liters || parseInt(form.liters) <= 0)
  ...
  const totalLiters = rows.reduce(
    (s, r) => s + parseInt(r.liters || 0),
    0
  );
  const totalCost = rows.reduce(
    (s, r) => s + parseInt(r.cost || 0),
    0
  );
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Liters",
      render: (v) => (
        <span className="text-mono text-sm text-light">
          {v} L
        </span>
      ),
    },
    ...
    <div className="space-y-5 animate-fade-up">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Summary
          label="Avg Cost / L"
          value={`₹${
            rows.length && totalLiters
              ? (totalCost / totalLiters).toFixed(0)
              : "0"
          }`}
          color="sky"
        />
        ...
        <div className="space-y-4">
          ...
          {form.liters && form.cost && (
            <div className="flex justify-between px-3 py-2.5 rounded-lg bg-plate/50 text-sm">
              ...
              <span className="text-mono text-jade">
                ₹
                {(
                  parseInt(form.cost) /
                  parseInt(form.liters)
                ).toFixed(2)}
              </span>
            </div>
          )}

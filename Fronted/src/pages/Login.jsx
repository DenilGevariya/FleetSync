    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex flex-col w-1/2 bg-hull border-r border-plate p-14 relative overflow-hidden">
        <div className="absolute inset-0 opacity-30"
          style={{ backgroundImage: "linear-gradient(rgba(38,56,80,0.5) 1px,transparent 1px),linear-gradient(90deg,rgba(38,56,80,0.5) 1px,transparent 1px)", backgroundSize: "40px 40px" }} />
        ...
        <div className="relative">
          <div className="flex items-center gap-3 mb-14">
            ...
            <span className="font-display font-extrabold text-snow text-2xl tracking-tight">FleetFlow</span>
          </div>

          <h2 className="font-display font-extrabold text-4xl text-snow leading-tight mb-4">
            Fleet Intelligence<br /><span className="text-amber">Command Center</span>
          </h2>
          <p className="text-ghost text-sm leading-relaxed max-w-xs">
            A centralized hub to manage your delivery fleet, track driver compliance, and monitor operational costs in real time.
          </p>
        </div>
        ...
        <div className="relative mt-auto grid grid-cols-2 gap-3">
          {[
            ...
            <div key={s.label} className="bg-plate/60 border border-wire/50 rounded-xl px-4 py-3">
              <div className="text-mono text-sm font-bold text-amber">{s.value}</div>
        ...
        <div className="w-full max-w-sm animate-fade-up">
          <div className="flex items-center gap-2 mb-10 lg:hidden">
            <div className="w-7 h-7 rounded-lg bg-amber/15 border border-amber/30 flex items-center justify-center">
              <svg className="w-3.5 h-3.5 text-amber" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M1 3h15v13H1z"/><path d="M16 8h4l3 4v4h-7V8z"/>
              </svg>
            </div>
            <span className="font-display font-bold text-snow text-lg">FleetFlow</span>
          </div>

          <h1 className="font-display font-bold text-snow text-2xl mb-1">Welcome back</h1>
          <p className="text-ghost text-sm mb-8">Sign in to your command center</p>

          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className="field-label">Email</label>
              <input type="email" className="field-input" placeholder="admin@fleetflow.io"
          ...
          <div className="mt-8 p-3 rounded-xl bg-plate/60 border border-wire/50">
            <p className="text-[11px] text-dim font-display font-semibold uppercase tracking-wider mb-2">Default credentials</p>
            <p className="text-mono text-xs text-ghost">admin@fleetflow.io</p>
            <p className="text-mono text-xs text-dim">password (from bcrypt hash in schema)</p>

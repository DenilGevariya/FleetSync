...
export default function Vehicles() {
  ...
  const [modal, setModal] = useState(null); // null | create | edit | status | confirm
  ...
  const load = () => {
    setLoading(true);
    vehiclesAPI.getAll().then((r) => setRows(r.data.data)).catch(() => toast.error("Failed to load vehicles")).finally(() => setLoading(false));
  };
  ...
  const validate = () => {
    ...
    if (!form.capacity || parseInt(form.capacity) <= 0) e.capacity = "Must be positive";
    setErr(e);
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      if (modal === "create") { await vehiclesAPI.create({ ...form, capacity: parseInt(form.capacity) }); toast.success("Vehicle added."); }
      else { await vehiclesAPI.update(sel.id, { ...form, capacity: parseInt(form.capacity) }); toast.success("Vehicle updated."); }
      setModal(null); load();
  ...
  const del = async () => {
    try { await vehiclesAPI.delete(sel.id); toast.success("Vehicle deleted."); setModal(null); load(); }
  ...
  const columns = [
    ...
    { key: "name",         label: "Vehicle",  render: (v, row) => (
      <div>
    ...
    { key: "capacity",     label: "Capacity", render: (v) => <span className="text-mono text-xs text-ghost">{v} kg</span> },
    ...
    { key: "created_at",   label: "Added",    render: (v) => <span className="text-xs text-dim">{new Date(v).toLocaleDateString("en-IN")}</span> },
    {
      ...
      render: (id, row) => can("MANAGER") && (
        <div className="flex items-center gap-1">
          <button className="btn-ghost text-xs" onClick={() => { setSel(row); setForm({ name: row.name, plate_number: row.plate_number, capacity: row.capacity }); setErr({}); setModal("edit"); }}>Edit</button>
  ...
  return (
    <div className="space-y-5 animate-fade-up">
      <PageHeader title="Vehicle Registry" sub={`${rows.length} vehicles · uses plate_number & capacity columns`}>
      ...
      <div className="card overflow-hidden">
        <DataTable columns={columns} data={filtered} loading={loading} empty="No vehicles yet. Add one to get started." />
      </div>

      {/* Create / Edit */}
      <Modal open={modal === "create" || modal === "edit"} onClose={() => setModal(null)} title={modal === "create" ? "Add Vehicle" : "Edit Vehicle"}>
        <div className="space-y-4">
          <I name="name" label="Vehicle Name" req />
          <I name="plate_number" label="Plate Number" req />
          <I name="capacity" label="Capacity (kg)" type="number" req />
        </div>
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-plate">
          ...
          <button className="btn-primary" onClick={save} disabled={saving}>
            {saving && <Spinner size="sm" />} {modal === "create" ? "Add Vehicle" : "Save"}
          </button>
        </div>
      </Modal>
      ...
      <Modal open={modal === "status"} onClose={() => setModal(null)} title="Change Status">
        ...
        <div className="space-y-2">
          {["AVAILABLE","IN_SHOP","RETIRED"].map((s) => (
            <button key={s} disabled={sel?.status === s || saving}
              onClick={() => setStatus(s)}
      ...
      <ConfirmModal open={modal === "confirm"} onClose={() => setModal(null)} onConfirm={del}
        title="Delete Vehicle" message={`Delete "${sel?.name}" (${sel?.plate_number})? This cannot be undone.`} />
    </div>
  );
}

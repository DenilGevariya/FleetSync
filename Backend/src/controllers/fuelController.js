...
const getAll = async (req, res, next) => {
  try {
    ...
    let sql = `
      SELECT f.*, v.name AS vehicle_name, v.plate_number
      FROM fuel_logs f
      LEFT JOIN vehicles v ON v.id = f.vehicle_id
    `;
    const params = [];
    if (vehicle_id) { sql += " WHERE f.vehicle_id = $1"; params.push(vehicle_id); }
...
const getOne = async (req, res, next) => {
  try {
    const result = await pool.query(
      `SELECT f.*, v.name AS vehicle_name, v.plate_number
       FROM fuel_logs f LEFT JOIN vehicles v ON v.id = f.vehicle_id
       WHERE f.id = $1`,
      [req.params.id]
    );
    if (!result.rows.length) return res.status(404).json({ success: false, message: "Fuel log not found." });
...
const create = async (req, res, next) => {
  try {
    ...
    if (!vRes.rows.length) return res.status(404).json({ success: false, message: "Vehicle not found." });

    const result = await pool.query(
      "INSERT INTO fuel_logs (vehicle_id, liters, cost) VALUES ($1,$2,$3) RETURNING *",

...
const getAll = async (req, res, next) => {
  try {
    ...
    let sql = `
      SELECT *,
        CASE WHEN license_expiry < CURRENT_DATE THEN true ELSE false END AS license_expired
      FROM drivers
    `;
    const params = [];
...
const getOne = async (req, res, next) => {
  try {
    const result = await pool.query(
      `SELECT *, CASE WHEN license_expiry < CURRENT_DATE THEN true ELSE false END AS license_expired
...
const create = async (req, res, next) => {
  try {
    ...
    if (!name || !license_number || !license_expiry) {
      return res.status(400).json({ success: false, message: "name, license_number and license_expiry are required." });
    }
    const result = await pool.query(
      "INSERT INTO drivers (name, license_number, license_expiry) VALUES ($1,$2,$3) RETURNING *",
...
const remove = async (req, res, next) => {
  try {
    const check = await pool.query("SELECT status FROM drivers WHERE id = $1", [req.params.id]);
    if (!check.rows.length) return res.status(404).json({ success: false, message: "Driver not found." });
    if (check.rows[0].status === "ON_TRIP") {
      return res.status(409).json({ success: false, message: "Cannot delete driver who is ON_TRIP." });
    }
